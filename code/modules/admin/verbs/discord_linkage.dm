/client/proc/link_ckey2discord()
	...
	if(!holder || !check_rights(R_DEBUG))
		return
	var/select = input(src, "Select type of data for link (and open DM)", "Data") in list("Account ID", "Name")
	var/list/data = list(
		"type"= "link",
	)
	var/selected
	switch(select)
		if("Account ID")
			selected = input(src, "Enter your ID", "Data")
			if(!selected)
				return
			data["id"] = selected
		if("Name")
			selected = input(src, "Enter your name (not server name, only account name)", "Data")
			if(!selected)
				return
			data["name"] = selected
	data["hash"] = md5(num2text(world.time))
	send2discordwh(data)
	if(input(src, "To confirm, enter here number which was sended to you in DM", "Confirmation") == data["hash"])
