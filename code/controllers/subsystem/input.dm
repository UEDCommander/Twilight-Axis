...
/datum/controller/subsystem/input/proc/get_client_from_list(list/clients, index)
	var/entry = clients[index]
	if(!istype(entry, /client))
		clients.Cut(index, index + 1)
		return null
...
/datum/controller/subsystem/input/proc/refresh_client_macro_sets()
	...
	for(var/i = clients.len; i >= 1; i--)
		var/client/user = get_client_from_list(clients, i)
		if(!user)
			continue
...
/datum/controller/subsystem/input/fire()
	...
	for(var/i = clients.len; i >= 1; i--)
		var/client/C = get_client_from_list(clients, i)
		if(!C)
			continue
